...
void evictBlockl2(cache* lcache, addr* ptr2) {
    addr *ptr = malloc(sizeof(addr));
    strcpy(ptr->address, ptr2->address);
    convertAddress(l2cache, ptr);
    for (int i = 0; i < lcache->assoc; i++) {
...
int readCachel2(addr *ptr2) {
    addr *ptr = malloc(sizeof(addr));
    strcpy(ptr->address, ptr2->address);
    convertAddress(l2cache, ptr);

    int setInx = ptr->setinx;
    int inx = checkSetFor(l2cache, ptr);
    //printf("find %s as 0x%x\n", ptr->address, ptr->addressVal);
    if (inx >= 0) {
        if (l2cache->fifo == 1) {
            for (int i = 0; i < l2cache->assoc; i++) {
                if (l2cache->sets[setInx].blocks[i].evictPos > l2cache->sets[setInx].blocks[inx].evictPos) {
                    l2cache->sets[setInx].blocks[i].evictPos--;
                }
            }
            l2cache->sets[setInx].blocks[inx].evictPos = l2cache->sets[setInx].currentinx - 1;
        }
        l2cache->cacheHit++;
	l2cache->sets[setInx].currentinx--;
	l2cache->sets[setInx].blocks[inx].valid = 0;
	for (int i = 0; i < l2cache->assoc; i++) {
	  if (l2cache->sets[setInx].blocks[i].evictPos > l2cache->sets[setInx].blocks[inx].evictPos) {
	    l2cache->sets[setInx].blocks[i].evictPos--;
	  }
	}
	l2cache->sets[setInx].blocks[inx].evictPos = -1;
    }
    else {
        l2cache->cacheMiss++;
        inx = -1;
...
void writeCachel2(addr *ptr2) {
  //printf("enter\n");
    ...
    if (inx >= 0) {
        for (int i = 0; i < l2cache->assoc; i++) {
            if (l2cache->sets[ptr->setinx].blocks[i].evictPos > l2cache->sets[ptr->setinx].blocks[inx].evictPos) {
                l2cache->sets[ptr->setinx].blocks[i].evictPos--;
            }
        }
        l2cache->sets[ptr->setinx].blocks[inx].evictPos = l2cache->sets[ptr->setinx].currentinx - 1;
        free(ptr);
        return;
    }
    //printf("L2 cache\n");
    if (inx == -2) {
        evictBlockl2(l2cache, ptr);
	//printf("evict block\n");
    }
    inx = getFreeBlock(l2cache, ptr);
    l2cache->sets[ptr->setinx].blocks[inx].valid = 1;
    l2cache->sets[ptr->setinx].blocks[inx].evictPos = l2cache->sets[ptr->setinx].currentinx++;
    //printf("block freed\n");
    strcpy(l2cache->sets[ptr->setinx].blocks[inx].address,ptr->address);
    l2cache->sets[ptr->setinx].blocks[inx].tag = ptr->Atag;
    free(ptr);
}
...
void evictBlockl1(cache *lcache, addr *ptr) {
    for (int i = 0; i < lcache->assoc; i++) {
        if (lcache->sets[ptr->setinx].blocks[i].evictPos == 0) {
            addr *ptr2 = malloc(sizeof(addr));
            strcpy(ptr2->address, lcache->sets[ptr->setinx].blocks[i].address);
            convertAddress(l2cache, ptr2);
            writeCachel2(ptr2);
            free(ptr2);
            lcache->sets[ptr->setinx].blocks[i].valid = 0;
            lcache->sets[ptr->setinx].blocks[i].evictPos = -1;
...
int readCachel1(addr *ptr) {
    ...
    if (inx >= 0) {
        ...
        strcpy(l1cache->sets[setInx].blocks[inx].address,ptr->address);
...
int main(int argc, char* argv[])
{
    ...
    //L2 Cache Properties
    l2cache->memSize = atoi(argv[5]);
    token = strtok(argv[6], ":");
    token = strtok(NULL, ":");
    l2cache->assoc = atoi(token);
    if (strcmp(argv[7], "fifo") == 0)
        l2cache->fifo = 0;
    else if (strcmp(argv[7], "lru") == 0)
        l2cache->fifo = 1;
    l2cache->blockSize = l1cache->blockSize;
    createCache(l2cache);
    
    dataFile = fopen(argv[8], "r");
